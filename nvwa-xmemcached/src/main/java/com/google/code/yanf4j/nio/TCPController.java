...
public class TCPController extends SocketChannelController {
	...
	protected void doStart() throws IOException {
		...
		if (this.connectionTime != 0 || this.latency != 0 || this.bandwidth != 0) {
			this.serverSocketChannel.socket().setPerformancePreferences(this.connectionTime,
					this.latency, this.bandwidth);
		}
		...
		setLocalSocketAddress((InetSocketAddress) this.serverSocketChannel.socket()
				.getLocalSocketAddress());
		this.selectorManager
				.registerChannel(this.serverSocketChannel, SelectionKey.OP_ACCEPT, null);
	}
	 ...
	 */
	private void closeAcceptChannel(SelectionKey sk, SocketChannel sc) throws IOException,
			SocketException {
		if (sk != null) {
			sk.cancel();
		}
