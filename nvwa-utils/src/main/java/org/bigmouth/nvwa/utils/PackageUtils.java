...
public class PackageUtils {
	...
	public static String[] findClassesInPackage(String packageName, List<String> included,
		...
		while (dirs.hasMoreElements()) {
			...
			if ("file".equals(protocol)) {
				findClassesInDirPackage(packageOnly, included, excluded, URLDecoder.decode(url
						.getFile(), "UTF-8"), recursive, vResult);
					...
					if (name.startsWith(packageDirName)) {
						...
						if ((idx != -1) || recursive) {
							...
							if (name.endsWith(".class") && !entry.isDirectory()) {
								String className = name.substring(packageName.length() + 1, name
										.length() - 6);
	...
	private static void findClassesInDirPackage(String packageName, List<String> included,
			List<String> excluded, String packagePath, final boolean recursive, List<String> classes) {
	...
	public static void main(String[] args) throws IOException {
		String[] clsNames = PackageUtils.findClassesInPackage("com.channel.transport.*",
